...
- [09 — Wire guardrail into build.py as gate](issues/09-wire-guardrail-into-build-py-as-gate.md) — verified `build.py` already gates the rule via `cargo nextest run --no-fail-fast`; rule is hard-failure (panics on violations), no `#[allow(...)]` annotations, and module doc already uses enforced language. `cargo test --test guardrails guardrails_inherent_impl_locality` and full `python build.py` are green.
- [01 — Build inherent impl locality rule](issues/01-build-inherent-impl-locality-rule.md) — re-added and landed. Two-pass AST check over `src/` collects `struct`/`enum`/`union` definitions by simple name, then flags inherent `impl Foo` blocks that are neither in the defining file nor under a folder named `snake_case(Foo)`. Excludes trait impls, `_tests.rs`/`_test.rs`/`main.rs`, and `#[cfg(test)] mod` subtrees. Unit tests cover same-file, named-folder split, wrong-folder split, cross-folder, trait-impl skip, generic self-type normalization, and cfg-test skip. Wired into `tests/infrastructure/guardrails/mod.rs` and auto-registered in `guardrails.md`.
- [12 — Refactor AppState render methods to module-per-type](issues/12-refactor-appstate-render-methods.md) — the last live violation surfaced by 01. Moved the `impl AppState` render helpers from `adapters/driving/http/layout/renderers/fragment_renderers.rs` into `adapters/driving/http/app_state.rs`, merged the tests into `app_state_tests.rs`, and deleted the now-empty `layout/renderers/` module. `guardrails_inherent_impl_locality` now reports zero violations on `src/` and `build.py` is green.
...
- [06 — Refactor Db* mappers to module-per-type](issues/06-refactor-db-mappers-to-module-per-type.md) — each stray `impl DbX` block consolidated from `storage/Xs.rs` into its defining `models/X.rs` (DbCharacter/DbGame/DbPersona/DbPromptPreset/DbSettings/DbWorld); moved methods widened `fn`→`pub(crate)`. User ruled out the `db_X.rs` rename (keeps `<concept>.rs`); user directed splitting the two multi-type def files `world.rs`→`world.rs`+`map.rs` and `message.rs`→`message.rs`+`swipe.rs` (module-per-type, beyond the violation set). `build.py` green; zero-violation verified structurally (rule itself still deferred per 01). 06's landing leaves 07 as the sole remaining blocker on 01 — the refactor cluster (03/04/05/06/08) is resolved; 07 still open.
- [07 — Resolve PromptPreset and PromptContext placement](issues/07-resolve-prompt-preset-cross-layer-placement.md) — `PromptPreset` behavior moved to domain (`assemble_text` + `render_field_parts` in `domain/model/prompt_preset.rs`), with application `render_preset_xml_parts` as a thin field→Section mapper; `PromptContext` definition moved into `application/prompting/assembler.rs` and re-exported via `mod.rs`. `build.py` green.
- [11 — Enforce folder cohesion in the inherent-impl-locality rule](issues/11-enforce-folder-cohesion-in-inherent-impl-locality-rule.md) — **leave cohesion as review policy; do not tighten the rule.**
- [10 — Document inherent impl locality rule](issues/10-document-inherent-impl-locality-rule.md) — resolved by events. `scripts/generate_guardrails_doc.py` already auto-generates the `inherent impl locality` row in `docs/diataxis/reference/coding_standards/guardrails.md` from the rule file's `///` doc comment, the established one-row pattern every guardrail rule uses. The ticket's specific instructions were stale on three points: its target path `docs/diataxi/reference/architecture_system.md` does not exist (real folder is `docs/diataxis/`, no `architecture_system.md` anywhere), its mirror reference `.scratch/free-fn-scanner-rules/` is gone, and its `[DOC: ...]` anchor instruction would break the `doc standards` guardrail that rejects DOC anchors in test files. No code or doc changes made.

<!-- 01 was previously deferred; it is now resolved above. -->

## Not yet specified

<!-- fog: suspected decisions that can't be pinned until the frontier advances -->

_Empty — the route to the destination is clear; all tickets are closed._

...
Re-created and landed the rule.

Files added/changed:
- `tests/infrastructure/guardrails/inherent_impl.rs` — two-pass AST check.
- `tests/infrastructure/guardrails/inherent_impl_tests.rs` — unit tests for the six acceptance cases plus a `#[cfg(test)] mod` skip case.
- `tests/infrastructure/guardrails/mod.rs` — registered the module, widened harness helpers to `pub(crate)`, and added the `guardrails_inherent_impl_locality` test entry.
- `docs/diataxis/reference/coding_standards/guardrails.md` — auto-regenerated by `scripts/generate_guardrails_doc.py` to include the new rule row.

Implementation details:
- Pass 1 builds a `HashMap<simple_name, defining_path>` from `ItemStruct`/`ItemEnum`/`ItemUnion`.
- Pass 2 flags inherent `ItemImpl` blocks whose self-type resolves to a known type, where the impl file is neither the defining file nor inside a folder named `snake_case(type)`.
- Self-type normalization handles `Type::Path`, `Reference`, `Paren`, `Group`, `Slice`, `Array`, and `Ptr`, returning the last path-segment ident; unknown shapes are skipped rather than flagged.
- Exclusions: trait impls (`item.trait_.is_some()`), `_tests.rs`/`_test.rs`/`main.rs` paths, and `#[cfg(test)] mod` subtrees (detected by a `cfg(test)` substring scan on the attribute).
- `to_snake_case` handles acronym boundaries (`DbCharacter`, `LLMMessage`, `DefaultApplicationService`).

The audit from this session surfaced exactly one remaining violation: `AppState` render helpers in `adapters/driving/http/layout/renderers/fragment_renderers.rs`. To keep `build.py` green, that violation was fixed in the same session via ticket **12**. After the fix, `guardrails_inherent_impl_locality` reports zero violations and `build.py` is fully green.

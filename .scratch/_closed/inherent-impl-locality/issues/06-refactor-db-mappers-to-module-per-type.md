...
**Incidental review fix (commit `14345cf`).** During post-resolution code review, `DbPromptPreset::into_preset` was flagged for a silent `_ => PresetType::System` fallback that masked unknown DB strings and duplicated the domain's `TryFrom<&str> for PresetType`. Fixed in the same commit: `into_preset` now returns `Result` and delegates to `PresetType::try_from`, propagating `EngineError::Parse` for bad data. This was a pre-existing bug, not a locality matter — folded into the commit because the code was already being touched. Not a map ticket.

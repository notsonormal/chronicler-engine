...
- World-toggle coupling (discovered in ticket-09 review, updated after the Strategy-A migration): `update_world_handler` is now read-modify-write — the stored `options_always_on` survives because the form does not model it. When a form checkbox lands (this ticket or elsewhere), add the field to `WorldForm` and patch it per-field, using the hidden-input pattern (`<input type="hidden" name="options_always_on" value="0">` before the checkbox): an unchecked HTML checkbox posts nothing, and a plain `bool` with serde default would silently clobber the stored `true` on every edit save.

...
fn draw_header(f: &mut Frame, area: Rect, room: &Room) {
    let header = Paragraph::new(Line::from(vec![
        ...
        Span::styled(&room.name, Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ]))
...
fn draw_story_log(f: &mut Frame, area: Rect, state: &GameState) {
    let history: Vec<Line> = state.narration_history.iter().map(|entry| {
        let mut spans = Vec::new();
        if let Some(sender) = &entry.sender {
            spans.push(Span::styled(format!("{}: ", sender), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)));
        }
...
fn draw_sidebar(f: &mut Frame, area: Rect, state: &GameState, room: &Room) {
    ...
    if let Some(path) = &room.image_path {
        ...
        f.render_widget(Block::default().borders(Borders::ALL).title(" Location (No Image) "), sidebar_chunks[0]);
    }
    ...
    let npc_sub_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints(vec![Constraint::Percentage(100 / npc_count); npc_count as usize])
        .split(sidebar_chunks[1]);

    for (i, npc_id) in room.npcs.iter().enumerate() {
        if let Some(npc) = state.npcs.get(npc_id) {
            if i < npc_sub_chunks.len() {
                if let Some(path) = &npc.sheet.image_path {
                    render_sidebar_image(f, npc_sub_chunks[i], path, &format!(" {} ", npc.sheet.name), true);
                } else {
                    f.render_widget(Block::default().borders(Borders::ALL).title(format!(" {} ", npc.sheet.name)), npc_sub_chunks[i]);
                }
            }
        }
    }
...
fn draw_input(f: &mut Frame, area: Rect, state: &GameState) {
    ...
    let input = Paragraph::new(input_text)
        .style(if state.tui_state.is_generating { Style::default().fg(Color::DarkGray) } else { Style::default() })
        .block(Block::default().borders(Borders::ALL).title(" Action (Enter command) "));
...
fn render_sidebar_image(f: &mut Frame, area: Rect, path: &str, title: &str, is_portrait: bool) {
    ...
    if !cache.contains_key(path) {
        if let Ok(mut raw_img) = image::open(path) {
            if is_portrait {
                // Bust crop: top 40%

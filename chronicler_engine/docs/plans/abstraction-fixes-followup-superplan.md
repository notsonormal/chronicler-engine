...
3. **Persistence policy** — pipeline uses `save_message_and_snapshot` (snapshot + newest message); arrival uses `save_snapshot` only. **Unverified: is message persisted in arrival path? Likely NO — possible bug, not just smell.**
...
- **G1 (persistence difference).** Is the arrival path's missing `save_message_and_snapshot` (constraint 3) intentional or a bug? If bug, deepened module gets one persistence policy; if intentional, two paths survive. **Verify in sub-plan.**

...
| 2 | `server` → `storage`, `narrative` | Driving adapters must not import driven adapters directly; route through ports | `src/adapters/driving/http/templates.rs` imports `LlmMessage`; `src/adapters/driving/http/templates.rs` + `view_models.rs` import `CheckResult` (driven adapter types) | After Phase 2 closes the leaks |

...
| 2 | `server` → `storage`, `narrative` | Driving adapters must not import driven adapters directly; route through ports | `src/adapters/driving/http/templates.rs` imports `LlmMessage`; `src/adapters/driving/http/templates.rs` + `view_models.rs` import `CheckResult`; `src/adapters/driving/http/fragments/actions.rs` + `src/adapters/driving/http/fragments/misc/text_check.rs` import `check_player_input` (4 driving → driven leak sites total) | After Phase 2 closes the leaks |
...
| 4 | `narrative` → `storage` | Driven adapters must not depend on other driven adapters (cross-adapter coupling forbidden) | `src/application/ports/llm_provider.rs` (`LlmBackend` trait default impls reaching into `Storage`); `src/application/agents/registry.rs`, `src/application/agents/quantifier/agent.rs` import `Storage` directly | After Phase 2 closes the leaks |

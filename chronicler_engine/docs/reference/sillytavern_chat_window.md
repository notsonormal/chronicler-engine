...
See `sillytavern_prompt_system.md` for the 8-layer prompt construction system. The chat window HTML is the **presentation layer** — it displays the conversation history (Layer 5: Chat History) and user input (Layer 6: User Input). The prompt assembly happens server-side via API calls; the HTML reflects the rendered conversation state.

...
- **Hexagonal architecture Phase 3 (polish + docs)** — Closed out `hexagonal-reorganization-plan.md` Phase 3 as doc-only polish. No `src/` changes. Two deviations from the original Phase 3 plan; Phase 1+2+3 now complete.
  - **Phase 3.1 — `engine/` subfolder decision:** `src/domain/engine/` kept as subfolder. Communicates types (`model/`) vs rules (`engine/`) separation; no port at boundary (application calls engine functions directly). Flattening rejected as churn for no architectural gain. Recorded in ADR-027 §3.1.
  - **Phase 3.2 — `DebugPort` rejected:** Single debug consumer (`src/adapters/driving/http/debug.rs`) + single debug surface = phantom port. `http/debug.rs` reaching into `ApplicationService` formalized as intentional guardrail exemption (no port). Recorded in ADR-027 §3.2 + `docs/architecture/guardrails.md` Phase 3.2 subsection.
  - **Phase 3.3 — Settings consolidation deferred:** Rename of `src/adapters/driven/storage/models/settings.rs` → `settings_row.rs` **deferred indefinitely**. Path disambiguation already solves the basename clash; renaming breaks the `models/` plain-table-name convention. `DbSettings` struct name preserved. Recorded in ADR-027 §3.3; plan Deviation 2.
  - **Phase 3.4 — Final docs + reconciliation:** ADR-027 §3.x subsections appended under `## Decision` (validate_adrs.py PASS). Storage-direct exemption count reconciled to **5 files** across `docs/architecture/system.md` + `docs/architecture/guardrails.md` to match ADR-027 (3 intentional persistence-boundary + 2 deferred-T2). ADR-027 "Positive" bullet "3 files only" → "5 files (3 intentional + 2 deferred-T2)". Stale plan-internal references (lines 567, 797) fixed to point to `docs/old-docs/archived-plans/` convention. Plan doc finalized: Phase 3 status marked complete, `## Phase 3 deviations` section appended (Dev 1 arch-lint stays deferred per Option B; Dev 2 settings rename deferred). Plan archived via `git mv` from `docs/plans/` to `docs/old-docs/archived-plans/hexagonal-reorganization-plan.md`. `hexagonal-deferred-arch-lint-rules.md` back-link updated to archived path.
  ...
  - **Plan:** `docs/plans/hexagonal-reorganization-plan.md` marked Phase 3 complete + 2 deviations; archived to `docs/old-docs/archived-plans/hexagonal-reorganization-plan.md`.

- **Phase 2 test quality cleanup + coverage gaps** — Implemented `docs/plans/archived/phase2-test-quality-and-coverage-gaps.md` per 14 locked decisions (commits `3b1ee5b` → `90af29b`, plus post-archive fix-up `3c5215e` → `<fix-up>`) on `hexagon-phase2`. Final build green: **1253 tests pass**; coverage **89.2% overall** (baseline 1225 tests / 87.1%).
  - **Axis C test quality (reviewer findings F1–F6, M1–M3):** removed 7 test fns (6 trivial + 1 fake regression guard); deleted `make_recording_recorder` dead helper; removed 3 unnecessary `.clone()` calls per-site; rewrote `StubChecker` from triple-nested `Option` to `VecDeque` queue; strengthened `provider_accessor_returns_injected_provider` with `Arc::ptr_eq`; added `provider().name()` assertions to 3 factory path tests.
  - **Axis D coverage gaps:** new `tests/http/server_impl_wiring.rs` (2 tests — happy-path spawn+cancel + bind-error propagation) covering previously-untested `server_impl.rs` (0% → 74.2%); `client_tests.rs` created then **removed** in post-archive fix-up (the two surviving tests only verified reqwest's error contract, not Chronicler behavior — no real coverage value, one was non-hermic hitting real OpenRouter DNS+TLS); extended `prompt_presets_fragment/fragments_tests.rs` (15 tests after D3 registration fix recovered 11 previously-orphaned tests) covering default+active combo and None-field paths (55.8% → covered); new `renderers/response_tests.rs` (15 tests) covering HTTP response helpers (74.6% → covered); extended `transport/response_tests.rs` (4 tests added) covering null-field fallbacks and whitespace input (78.5% → 78.5% — D7 reclassified PARTIAL; target `handle_response` defers to integration testing, see plan addendum).
  ...
  - **Post-archive fix-up pass 1 (2026-07-03):** addressed Review 1 + Review 2 P1/P2/P3 findings: deleted orphan scout artifact `context.md` from repo root; renamed misnamed `run_server_serves_request_and_returns_404_for_unknown_route` → `run_server_spawns_and_can_be_cancelled` (test name now matches assertion); deleted duplicate `call_ollama_compiles_and_runs`; deleted 2 no-op `text_check_factory_tests` (still assert nothing after polish); dropped `async` from `build_test_resources` (no `await` interior); fixed `AGENTS.md` typo (Mininmax → Minimax) + added trailing newline; structural fix to `RecordingForensics::save_llm_message` — counter now increments on entry (counting attempts, not just successes), eliminating the silent undercount footgun; moved orphaned `IssueKind` Display test from unregistered `types_tests.rs` to `text_checker_tests.rs` (ran for first time); deleted dead `src/test_support/forensics.rs` + `forensics_tests.rs` (260 LOC of half-built tracing-layer infra never wired in, never produced `tmp/diagnostics/` JSON — ADR-012 SQLite `llm_messages` path covers the same diagnostic need); updated `docs/diagnostics/DEBUGGING.md`, `docs/architecture/system.md`, `docs/system/llm_processing.md`, `docs/plans/observability-and-forensics-plan.md`, `docs/reviews/docs-consistency-report.md` to reflect the deletion; added structural guard to `scripts/check_test_structure.py` that verifies every `*_tests.rs` file has a matching `mod` declaration in its module root (catches the recurring orphaned-tests class of bug).
  - **Post-archive fix-up pass 2 (2026-07-03):** deleted `client_tests.rs` entirely (both surviving tests only verified reqwest's error contract on bad URLs — zero Chronicler-specific behavior asserted; one was non-hermic, hitting real OpenRouter DNS+TLS with a fake API key on every run, violating the plan's locked "do NOT hit real APIs" decision and AGENTS.md's `no-internet` network convention). D2 status changed from "complete" to **removed** — the original 40.9% coverage gap on `transport/client.rs` was never actually closed by these tests; marked as real follow-up (bring `client.rs` to ≥80% via hermic tests asserting actual Chronicler behavior). Coverage held at 89.2% (the deleted `Err(...)` arms are already covered elsewhere in `transport/{request,response}_tests.rs`).

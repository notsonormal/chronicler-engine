...
> Status: Implemented. See [ADR-014](../adr/adr-014-action-pipeline.md) for rationale (including the `PipelineRun<'a>` borrow structure and `PipelineInputs` ownership choice) and [ADR-027](../adr/adr-027-hexagonal-architecture-migration.md) for the ports/traits collapse.

## Objective

The action pipeline orchestrates the FreeAction lifecycle: pre-snapshot, narrate, post-generation agents, engine commit, trigger continuation, finalize. It unifies the normal action flow and the retry flows. Pipeline phases run inside `spawn_blocking` via `spawn_pipeline_task`; the pipeline instance is constructed once at startup and shared by `Arc`.
...
Cancel checkpoints (red diamonds): after the LLM narrator call returns, at the start of trigger continuation, after the trigger LLM call returns. Each is an α-check returning `Err(PhaseError::Cancelled)` — see [Cancellation](#cancellation).
...
The state field IS the error channel: `state.narrative.input_buffer.status` carries phase failures, and the UI polls it via `get_generating_status`. `Err` at the `spawn_blocking` boundary only signals cancellation. `run_from_input` finalizes and returns `Ok(())` whenever `status.error_message()` is set, so state is always persisted and the UI shows the error through the existing polling path.

Two error variants travel via `Result<_, PhaseError>`:

- **`PhaseError::Cancelled`** — the only variant that crosses the `spawn_blocking` boundary. All other variants are dropped there because `status` already carries the error.
- **`PhaseError::PersistFailed { label, source }`** — constructed by `persist_snapshot_or_err` at four snapshot sites (`pre-main`, `pre-event`, `post-trigger`, `post-engine`); the helper writes `GenerationStatus::Error(...)` + persists state before returning the `Err`, so the UI surface stays consistent even when `run_from_input` returns `Err(...)` upstream. The four sites do not handle the `Err` uniformly — see [ADR-032](../adr/adr-032-phaseerror.md).

The remaining variants (`NarratorFailed(String)`, `TriggerMissing`, `SnapshotMissing`) are reserved — not constructed in `src/`. See [ADR-032](../adr/adr-032-phaseerror.md).

`phase_finalize` resets `status` to `Idle` and `phase` to default UNLESS `status` is already `Error`, guaranteeing the UI never sees a stuck `Generating` after the pipeline returns. The retry side mirrors this load-fresh + set-status + persist structure via `save_retry_error`, but writes `Error(...)` (never resets to `Idle` or clears `phase`).
...
`handle_cancellation` is the single cleanup point: load fresh state, set `Idle`, clear `phase`, persist, return `Err(PhaseError::Cancelled)`. The retry side also runs inline cleanup, duplicating the central path — see [ADR-032](../adr/adr-032-phaseerror.md).
...
- **Event retry** (`retry_event_continuation`) calls the `pub(crate)` wrapper `pipeline.phase_trigger_continuation`, then `PipelineRun::reconcile_post_trigger_npcs` to re-quantify NPC state. Regenerates only the continuation text using the stored `StoredTriggerContext` (carried by `state.narrative.last_trigger`); does not rerun main narration or quantification.

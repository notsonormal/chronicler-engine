...
The initial room is sourced from the active scenario's `starting_room_id` (default `"start"`). When a game begins, bootstrap resolves `world.default_scenario()` and sets the player's `current_room_id` from `scenario.starting_room_id`. If no scenarios exist, the fallback is `"start"`.

...
- **Retrigger Event** appears on a narration swipe when its restored snapshot contains `last_trigger` and there are no event messages after it. Clicking it runs the trigger continuation flow via `ActionPipeline::phase_trigger_continuation()` → `ActionPipeline::reconcile_post_trigger_npcs()` → `phase_finalize()` from that snapshot state without rerunning the main narration.

...
- **`application_service`**: `DefaultApplicationService` is the application-tier orchestrator. Holds process refs (storage, preset_storage, settings, cancel_token, is_generating) by `Arc` and exposes them via accessors (`storage()`, `preset_storage()`, `settings()`, `cancel_token()`, `is_generating()`, `game_service()`). Long-lived (single instance per process), not per-op.

...
- **`application_service`**: Thin orchestrator struct (`DefaultApplicationService`) with game lifecycle operations inlined (`create_game`, `switch_game`, `delete_game`, `list_games`, `current_game_id`, `reset`, worlds CRUD, `list_personas`). Contains `process_action` entry point with self-healing stale-`Generating` detection, `spawn_pipeline_task` helper, and `GenerationGuard` RAII helper for `is_generating` flag cleanup. `ApplicationError::is_user_displayable()` enables type-driven error branching — validation errors and `WorldHasGames` domain constraints are inline-displayable; engine errors use `app_err_to_response()`.

...
- **`persistence_gate`** (T2 ticket 02): `application/persistence_gate/` module owns the game `Arc<Storage>` + `Arc<PresetStore>` newtype and bundles persistence helpers (`load_world_snapshot`, `world_snapshot_or_empty`, `load_or_fresh`, `load_expecting_valid_state`, `save_state`, `save_message_and_snapshot`, `delete_and_remove_message`, `load_messages_with_swipes`, `load_messages_into_state`, `build_fresh_initial_state`, `load_messages`, `update_message_text`, `find_retry_anchor`, `set_game_id`). `PersistenceGate` struct (`gate.rs`) + `WorldSnapshot` DTO (`dto.rs`) are the seam between orchestration and SQLite. Per ADR-027, `gate.rs` imports `Storage` directly (grandfathered in `tests/infrastructure/guardrails/layers.rs`). `DefaultApplicationService` holds a `pub(crate) persistence_gate: Arc<PersistenceGate>` and forwards each method as a thin delegate; faade-first preserves ~30 call sites this wave (caller-site migration deferred to a follow-up PR).
- **`generation_gate`** (T2 ticket 03): `application/generation_gate/` module owns the per-process `CancellationToken` + `Arc<AtomicBool>` (`is_generating` cache from ADR-030) and the generation-slot orchestration (`start_action` moved from `process_action` body, plus `heal_stale_generating`, `claim_generation_slot`, `release_generation_slot`). `DefaultApplicationService` holds a `pub(crate) generation_gate: GenerationGate` field; the faade's `process_action` / `continue_narration` plus the three slot helpers become 1-line delegates. The faade accessors `is_generating()` and `cancel_token()` stay on `DefaultApplicationService` (used by HTTP handlers + bootstrap) and forward to `generation_gate.is_generating()` / `generation_gate.cancel_token()`. Per ADR-030 single-writer rule, `is_generating` is mutated only by (a) the gate's CAS `false → true` at slot claim and (b) the gate's `release_generation_slot`; the `true → false` RAII fallback still lives in `application::generation_guard::GenerationGuard::Drop`. Motion is purely structural (the `start_action` body stays at 47 lines pending the T6 gate amendment that will split it). The AppState dual-token question (AppState holds its own `Arc<RwLock<CancellationToken>>` separate from `DefaultApplicationService`'s token; see map §Not-yet-specified) is **deferred** — preserved as-is here, graduated to a follow-up ticket.
- **`application_service`**: `DefaultApplicationService` is the application-tier orchestrator. Holds process refs (storage, preset_storage, persistence_gate, settings, generation_gate, game_service) by `Arc` and exposes them via accessors (`storage()`, `preset_storage()`, `settings()`, `cancel_token()`, `is_generating()`, `game_service()`); `cancel_token()` / `is_generating()` forward to `generation_gate`. Long-lived (single instance per process), not per-op.
  - Shared persistence helpers live as methods on `DefaultApplicationService`: `load_or_fresh`, `load_expecting_valid_state`, `save_message_and_snapshot`, `delete_and_remove_message`, `load_messages_with_swipes`, `load_messages_into_state`, `load_messages`, `update_message_text`, `migrate_swipes`, `map_llm_error` (free fn). Replaces the now-removed per-op context struct + `context.rs` module.
  - `save_message_and_snapshot(&mut self, state)`: Saves a snapshot and immediately persists the newest unpersisted message with the snapshot ID. Messages are persisted as they are created; there is no batching or `committed` flag.
  - Game lifecycle operations: `create_game`, `switch_game`, `delete_game`, `list_games`, `current_game_id`, `reset`, worlds CRUD. `process_action` entry point has self-healing stale-`Generating` detection and uses a `GenerationGuard` RAII helper for `is_generating` flag cleanup. `process_action` spawns its blocking task via the shared `application::spawn_pipeline_task` helper.
  - `ApplicationError::is_user_displayable()` enables type-driven error branching — validation errors and `WorldHasGames` domain constraints are inline-displayable; engine errors use `app_err_to_response()`.
...
- **`message_editing.rs`**: Free fns for message editing operations - `switch_swipe`, `edit_history`, `delete_last`, `retry`, `retrigger`. `retry` and `retrigger` take `Arc<DefaultApplicationService>` and spawn their pipeline task via the shared `application::spawn_pipeline_task` helper. `switch_swipe`/`edit_history`/`delete_last` take only `&DefaultApplicationService` (no separate `game_service` argument).
...
- **`spawn.rs`**: `pub(crate) fn spawn_pipeline_task(app, f: F)` — thin wrapper that moves the `Arc<DefaultApplicationService>` into a `spawn_blocking` closure and hands `&DefaultApplicationService` to `f`. Cancel-check + `GenerationGuard` lifetime stay inside each caller's closure (zero behavior change).
- **`arrival_service.rs`**: Arrival narration use case. `ArrivalTaskContext` struct (~9 fields — owns `Arc<DefaultApplicationService>`, prompt preset, NPC lists, `LlmCallRecorder`, room id, token/length params) + `run()` method (loads snapshot → builds prompt → calls LLM narrator → persists via `app.save_message_and_snapshot`). Owns a clone of the app's `Arc` so it can spawn `spawn_blocking` directly without the shared helper. Test-only constructor `new_for_test` + `run_sync` entrypoint.
...
- **`game_service`**: `GameService` struct (renamed from `DefaultGameService`) holds the three injected `Arc`s (prompt assembler, LLM recorder, agent registry) plus `ActionPipeline`. Exposes `pipeline()` accessor returning a fresh `ActionPipeline` (callers no longer reach into `prompt_assembler` / `llm_recorder` / `agent_registry` fields to build one). External callers go through the `DefaultApplicationService` entry points (`process_action`, `load_messages`, `load_or_fresh`, etc.); only the `ActionPipeline` internals call the impl functions directly. No trait impl — `ActionPipelineBackend` deleted.
...
**Context Loading Pattern (ADR-025):** Handlers load the world context on-demand from DB based on the active game's `world_key` and route it through `app.load_or_fresh()` / `app.load_expecting_valid_state()` etc. All handlers MUST propagate errors — never silently swallow with defaults. The HTTP-layer `ctx_or_error()` helper in `renderers.rs` is the standard seam for consistent error-to-response mapping.
...
- **`fragments`**: HTML fragment generators for HTMX partial updates. Split into submodules:
  ...
  - **`renderers`**: HTML rendering helpers, markdown→HTML via `pulldown-cmark`. Exports `ctx_or_error()` helper for consistent `Result`-propagation from application-tier operations to HTTP responses.

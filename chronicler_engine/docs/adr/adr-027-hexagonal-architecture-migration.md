...
- **2026-08-01**: Facade-deletion work completed: `DefaultApplicationService` and `src/application/application_service.rs` removed; collaborators (`ActionPipeline`, `GameCatalogue`, `WorldPersonaCatalogue`, `GameViewQuery`, `GenerationGate`) wired directly through `WiredApp`/`AppState`. Current storage-direct exemption list: `src/application/persistence_gate.rs` (intentional boundary), `src/application/agents/registry.rs` and `src/application/agents/quantifier/agent.rs` (deferred to G1-B). `game_catalogue.rs` and `world_persona_catalogue.rs` reach Storage via `PersistenceGate::storage()`; `context.rs`, `game_service.rs`, and `application_service.rs` deleted. The accepted body of this ADR reflects the state at the time of the decision; see current reference docs and `tests/infrastructure/guardrails/layers.rs` for the live allowlist.

...
### Lock-order fix (post-07 cleanup)

The original Ticket 07 amendment above states that "the same path that mutates the per-game registry also updates the `AtomicBool` projection". That claim was true only in the code-path sense — the `AtomicBool.store()` happened on the same code path as the registry mutation, but *after* the registry write-lock guard had dropped. A release-then-claim interleaving across two games could therefore produce a TOCTOU window: A's `release_owned_slot` scans the registry inside the write lock, drops the lock, then stores `false` on the projection; B's claim on another game stores `true` in between; A's pending `store(false)` then clobbers B's `store(true)`, leaving the projection `false` while B's registry slot is still `Generating`.

The cleanup closes this window by moving the projection `store(true)` (on claim) and `store(false)` (on release) *inside* the registry write-lock scope, so the registry mutation and the projection mutation are atomic with respect to concurrent claims and releases on other games. A's `release_owned_slot` now sees (via the ownership check) that B owns the slot and bails out as a no-op; even when a release legitimately clears its own slot, the `store(false)` runs under the same lock that just observed `any_other_generating() == false`, so the projection cannot be clobbered by a later writer.

The heal path (`heal_stale_generating`) is consistent with this invariant. Its outer `is_generating.load()` is only an optimization to skip the write-lock acquisition when trivially idle; inside the registry write lock, the atomic is re-read as authoritative (atomic writes are now performed under this same lock, so the load is stable while the lock is held). If the in-lock re-read says `true`, a concurrent claim has raced in and the apparent stale slot is in fact a live claim — the heal returns without clearing. Only when the in-lock atomic is `false` is `slot.is_generating()` treated as authoritative evidence of a stale entry and cleared.

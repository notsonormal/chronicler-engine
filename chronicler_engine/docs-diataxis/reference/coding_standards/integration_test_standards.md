...
- `tests/integration/application/wiring.rs` is the only Pattern-1 test that does NOT use `execute_action_impl` — it drives the recorder directly via `LlmCallRecorder::complete(...)` and asserts the message landed in the *real* `Storage`. This test catches the silent-fallback regression where the prod factory (`bootstrap::wiring::build_game_service`) falls back to `NoopForensics` instead of writing to `Storage`.

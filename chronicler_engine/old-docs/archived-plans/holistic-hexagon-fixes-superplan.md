...
- Source investigation: `chronicler_engine/tmp/holistic-hexagon-investigation-second-opinion.md` (2026-07-05)
- Scout audit reports: H1 (axum usage in `application_service_tests.rs`), H6 (3 `phase_trigger_continuation` sites), H8 (`LayeredPromptAssembler` + `LayeredBackend` call sites) — all 2026-07-06, read-only, saved in session memory
- Sibling plans: `abstraction-fixes-followup-superplan.md`, `reliability-and-cancellation-plan.md`, `t1-error-model-unification.md`, `t5-type-collapses.md`, `t10-low-priority-cleanup-bundle.md`
- Constrained by: ADR-018 (application-service, deliberate-defer of `ArrivalTaskContext`), ADR-027 (storage-direct exemption)
...
- H3 dropped — phantom abstraction; 9 read sites with same idiom is not architectural drag.
- H0 split into 2 ADRs (ADR-028 `HttpError` boundary, ADR-029 `is_generating` invariant) — keeps audit trail clean.
- H2 rewritten as Option D (lifetime separation): service singletons → `AppState` (axum extractor); domain data → per-op `WorldView<'a>`; `GameServiceContext` deleted or shrunk to `WorldView`. Addresses root cause (singleton/entity lifetime mix), not symptom (method count).
- H1 SubTask 1.1.2 default = explicit impl (not blanket).
- H5 narrowed: extract pure-fn `build_narration_prompt` only; no shared `narrate_into_state` (false-dedup risk — paths not 1:1). Depends on H4 dropped; moved to Phase 2.
...
- **H0 (2 SP) — 2 ADRs:** ADR-029 (`HttpError` trait boundary: trait in `application/`, `IntoResponse` impl in `adapters/driving/http/`, explicit impl not blanket) + ADR-030 (`is_generating` dual-source invariant: AtomicBool = cached view, single-writer rule). ADR-028 is taken (`test-module-header-convention`). Blocks H1, H4.
...
| Finding | Class | Notes |
|---|---|---|
| C.2 | closed | Quantifier forensics fix landed 2026-07-06 |
| C.3 | closed | `GenerationGuard` single definition at `application/generation_guard.rs:10` |
| C.17 | closed | `run.rs` decomposed into `prepare_data`/`prepare_state`/`start_server` |
| B.1 | fabricated | `prompt_presets` has no `game_id`; "delete-game kills preset" risk doesn't exist. Excised. |
| A.1–A.5 | verified-truth | Existing architecture description, no action. |
| B.2 | YAGNI-deferred | ADR-027 intentional. |
| B.3 | documentary | Factory vs holder distinction; trivial. |
| B.6 | intentional | Persisted-via-JSON; doc accurate. |
| D.4 | closed | `MockBackend` signals verified; no fabricated quote. |
...
- [ ] #### Task 0.1: H0 — Two ADRs (2 SP)
  - [ ] ##### SubTask 0.1a: Draft ADR-029 `HttpError` trait boundary (1 SP) — scope: trait lives in `application/`; `IntoResponse` impl lives in `adapters/driving/http/`; explicit impl over `ApplicationError` (not blanket). ADR-conflict check vs ADR-018 + ADR-027. Verify: ADR file exists at `chronicler_engine/docs/adr/adr-029-http-error-boundary.md`, no conflicts logged. Note: ADR-028 is taken (`adr-028-test-module-header-convention.md`).
  - [ ] ##### SubTask 0.1b: Draft ADR-030 `is_generating` dual-source invariant (1 SP) — scope: AtomicBool = cached view of persisted `GenerationStatus`; single-writer rule = only `ApplicationService` mutates both in same critical section. ADR-conflict check vs ADR-018 + reliability plan R2. Verify: ADR file exists at `chronicler_engine/docs/adr/adr-030-is-generating-invariant.md`, no conflicts.
...
- [ ] #### Task 1.1: H1 — Move `IntoResponse` to adapter (5 SP)
  ...
  - [ ] ##### SubTask 1.1.2a: Lock decision in ADR-029 (already in Task 0.1a): explicit impl `impl IntoResponse for ApplicationError` in adapter (not blanket). Default. (0 SP — covered by H0)
...
- [ ] #### Task 3.1: H8 — Rename `Layered*` (3 SP)
  - [ ] ##### SubTask 3.1.1: Rename `LayeredPromptAssembler` → `PromptAssembler` across 4 files (`narrative_prompt/mod.rs` re-export, `game_service.rs` use + struct field + 2 constructions + fn param, `arrival_service.rs` construction + `.assemble()`, `action_pipeline/pipeline.rs` use + struct field + fn param). Verify: `grep -rn LayeredPromptAssembler chronicler_engine/src/` returns 0 hits. (2 SP)
...
Phase 0 → Phase 1 (H1 → H2 → H4) → Phase 2 (H5 → H6 → H7) → Phase 3 (H8 → H9). Each sub-plan locks its own scope and must stop-and-report deviations per `AGENTS.md` "Plan Adherence" rule.
...
- This super-plan does not dictate implementation; sub-plans own implementation detail. Where a sub-plan deviates, stop and report per `AGENTS.md` "Plan Adherence" rule.

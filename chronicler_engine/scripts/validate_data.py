...
def validate_all():
    ...
    for world_file in data_dir.glob("worlds/*/world.json"):
        try:
            ...
            print(f"PASS: {world_file}")
        except jsonschema.exceptions.ValidationError as e:
            print(f"FAIL: {world_file}\n{e.message}")
            errors += 1

    # Validate maps
    for map_file in data_dir.glob("worlds/*/map.json"):
        try:
            data = load_json(map_file)
            jsonschema.validate(instance=data, schema=map_schema)
    ...
    for char_file in data_dir.glob("personas/*.json"):
        try:
            ...
            jsonschema.validate(instance=data, schema=char_schema)
            print(f"PASS: {char_file}")
        except jsonschema.exceptions.ValidationError as e:
            print(f"FAIL: {char_file}\n{e.message}")
            errors += 1

    # Validate characters
    for char_file in data_dir.glob("characters/**/*.json"):
        try:
            data = load_json(char_file)
            jsonschema.validate(instance=data, schema=char_schema)
            print(f"PASS: {char_file}")
    ...
    for world_file in data_dir.glob("worlds/*/world.json"):
        ...
        def check_asset(asset_path, context):
            if asset_path:
        ...
        errors += check_asset(world_data.get("default_room_image"), f"{world_file}")
        ...
        if map_file.exists():
            for region in map_data.get("overworld", {}).get("regions", []):
                for room in region.get("rooms", []):
                    errors += check_asset(room.get("image_path"), f"Map {map_file.name} Room {room.get('id')}")
                    for npc_id in room.get("npcs", []):
                        npc_file = chars_dir / f"{npc_id}.json"
                        if not npc_file.exists():
                            print(f"FAIL: Map {map_file.name} Room {room.get('id')}\n  References missing NPC '{npc_id}' ({npc_file})")
                            errors += 1

        if chars_dir.exists():
            for char_file in chars_dir.glob("*.json"):
                char_data = load_json(char_file)
                errors += check_asset(char_data.get("profile_image"), f"{char_file}")
                errors += check_asset(char_data.get("headshot_image"), f"{char_file}")
    ...
    for char_file in data_dir.glob("personas/*.json"):
        char_data = load_json(char_file)
        def check_persona_asset(asset_path, context):
            if asset_path:
                full_path = base_dir / asset_path
                if not full_path.exists():
                    print(f"FAIL: {context}\n  Asset not found: '{asset_path}'")
                    return 1
            return 0
        errors += check_persona_asset(char_data.get("profile_image"), f"{char_file}")
        errors += check_persona_asset(char_data.get("headshot_image"), f"{char_file}")

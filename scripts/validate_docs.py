"""Validate markdown docs + DOC anchors under docs/.
...
def check_doc_anchors(report: FileReport, path: Path, engine_root: Path) -> None:
    ...
    reference_root = (repo_root / "docs" / "diataxis" / "reference").resolve()
    for lineno, line in enumerate(text.splitlines(), start=1):
            ...
            try:
                ...
                report.violations.append(
                    Violation(
                        ERROR,
                        "BROKEN_DOC_ANCHOR",
                        f"Line {lineno}: DOC anchor must resolve under docs/diataxis/reference/: `{target}` ({rel})",
                    )
                )
                continue
...
def scan_file(
    ...
    `engine_root` is the repo root; the file's parent path
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate markdown docs under docs/diataxis/."
        ),
    )
    parser.add_argument(
        "--path",

...
fn make_service() -> crate::application::pipeline::ActionPipeline {
...
pub(super) fn insert_message_with_swipe(
    _app: &AppState,
...
pub(super) fn add_input_and_save(
    app: &AppState,
    storage: &crate::adapters::driven::storage::Storage,
    text: &str,
) -> u64 {
...
pub(super) fn add_narration_and_save(
    app: &AppState,
    storage: &crate::adapters::driven::storage::Storage,
    text: &str,
) -> u64 {
...
pub(super) fn setup_event_flow_without_trigger(
    app: &AppState,
    storage: &crate::adapters::driven::storage::Storage,
) {
    let _ = add_input_and_save(app, storage, "test input");
...
async fn test_retry_last_response_cancelled_at_phase_boundary() {
    ...
    assert_eq!(
        state.narrative.input_buffer.status,
        GenerationStatus::Idle,
        "Cancelled retry must reset status to Idle, got {:?}",
        state.narrative.input_buffer.status
    );
    assert_eq!(

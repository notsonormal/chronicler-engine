...
12. Run the subagents for each of the 4 skills `/thermo-nuclear-code-quality-review`, `/code-review`, `/test-police`, `/antipattern-checker`. Instruct `/thermo-nuclear-code-quality-review`, `/code-review`, and `/antipattern-checker` to not run tests or to build, as we've already done both and they can't all `build.py` as the same time. The `/test-police` does not need this instruction as re-running the build/tests is part of its workflow.

...
- **DOC anchor cutover → strict full-path form** (plan: `old-docs/archived-plans/doc-anchor-cutover-strict-full-path-with-validator-enforceme.md`). Cut over all ~182 production `[DOC: ...]` anchors from legacy `docs/system|architecture|reference/<name>.md` paths to the new full-repo form `chronicler_engine/docs/diataxis/reference/<area>/<name>.md`. Added four new rules to `scripts/validate_docs.py`: `BROKEN_DOC_ANCHOR` (path-form + target-missing variants), `TEST_SUPPORT_ANCHOR_FORBIDDEN`, `TEST_FILES_ANCHOR_FORBIDDEN`, `TEST_SUPPORT_SUMMARY_REQUIRED`. Anchors are scanned unconditionally (no opt-out flag). Stripped `[DOC: ...]` lines from six `src/test_support/*.rs` files; line 1 now `//! <summary>`. Updated the Rust guardrail predicate at `tests/infrastructure/guardrails/structure.rs` from `docs/` to `chronicler_engine/docs/diataxis/reference/` (with `..` traversal rejection). Section-suffix form (`— section "..."`) is unsupported — path-only anchors.
- **DOC anchor restriction to `reference/` only.** Tightened both the Python validator and the Rust guardrail predicate: anchors must resolve under `chronicler_engine/docs/diataxis/reference/...`; `explanation/`, `how-to/`, `tutorials/` targets are rejected with `BROKEN_DOC_ANCHOR`. Remapped 18 LLM-adjacent files from `explanation/prompt_system_design.md` → `reference/narrative/prompt_system.md`, and 6 Harper text-check files from `reference/coding_standards/testing.md` → `reference/game_flow.md` (where text-check is actually documented).
...
- **T4 ticket 03: `run_from_input` body linearization** (wayfinder map: `.scratch/t4-phase-error/`). Body refactor only — no type churn, no signature changes. Collapsed the 5 mixed error styles (early-return, bool-check-via-helper, `tracing::error!`, `save_retry_error` patterns, `map_cancelled` wrapper) left in `ActionPipeline::run_from_input` after ticket 02's type migration into one linear `match PhaseError` pattern.
  - Added `ActionPipeline::finalize_phase_error(run, e)` — single seam that maps each `PhaseError` variant to its user-facing message, loads fresh state (since phase calls take state by value and drop it on `Err`), sets `GenerationStatus::Error(msg)`, and runs `phase_finalize`. `Cancelled` is matched out before the helper.
  - Added `ActionPipeline::load_world_bundle(app, started_for)` — extracts the inline `match (|| { ... })()` IIFE that loaded game/world/persona/npcs. Returns `Result<_, EngineError>`.
  - Flipped `phases.rs::error_return` per ADR-032 recommendation: returns `Err(PhaseError::NarratorFailed(msg))` instead of `Ok((state, empty, ...))`. Side effects preserved (set Error status, persist before Err). Eliminates the post-call `status.error_message().is_some()` dead check in the orchestrator.
  - `phase_engine_commit` `EngineError` wrapped at the orchestrator call site into `PhaseError::PersistFailed { label: "engine commit", source: e }` (no signature change).
  - 3 deliberate behavior changes per ticket 03 spec (all align with `"Error branches: set GenerationStatus::Error + persist + return Ok(())"`): `phase_pre_main_snapshot` PersistFailed now consumed (was `?`-propagated); `persist_snapshot_or_err` failure now sets Error status (original left status unchanged); `phase_trigger_continuation` non-Cancelled failure now finalizes + `Ok(())` (original returned `Err(e)` silently dropped by `execute_action_impl`).
  - `python build.py` green: 1255 tests passed, 2 LLM skipped, 0 failed. Cargo fmt clean, clippy clean. Coverage 86.7% (pipeline.rs 85.3%, phases.rs 82.3%).
  - LOC target ≤50 missed (body is 122 LOC) — call-site verbosity (6-arg `phase_engine_commit` + `phase_trigger_continuation_with_cancel_handling` calls) dominates. Linear structure achieved; further compression would require a forbidden `From<PhaseError>` impl.
  - Surfaced for ticket 04: `ActionPipeline::finalize_phase_error` is reusable by `retry_last_response_impl` (also goes through `ActionPipeline`). Reduces 04's scope.

- **T4 ticket 04: retry orchestrator linearization + 3 save-state paths reconciled** (wayfinder map: `.scratch/t4-phase-error/`). Body refactor on top of ticket 02's type surface + ticket 03's `finalize_phase_error` seam. **Amended ADR-032**: added `PhaseError::FetchFailed(String)` variant for retry precondition fetch failures (game/world/persona bundle load in `retry_event_continuation`). Rationale recorded in ADR-032 §"Why `FetchFailed` was added in ticket 04": distinct recovery policy from `NarratorFailed` (precondition check vs mid-flow LLM failure); payload `e.to_string()` preserves canonical `EngineError` displays (e.g. `Game not found: 999`).
  - `retry_last_response_impl` linearized into one linear `match PhaseError` at the bottom. `Err(PhaseError::Cancelled) => { load_or_fresh → Idle → save_state }` preserves the cancel-handling contract; `Err(e) => { construct PipelineRun → ActionPipeline::finalize_phase_error(&run, e) }` consumes every other variant through the same seam ticket 03 added. `PipelineRun` constructed inside the `Err(e)` arm only — happy path stays branchless. 5 precondition failures (`load_messages`, `find_retry_anchor`, `load_snapshot_by_id` Ok-None/Err, `last_input_text`) now route through a new private `retry_persist_error(app, msg)` helper (extracted from the deleted `save_retry_error`). `ActionPipeline::finalize_phase_error` + `load_world_bundle` made `pub(super)` so retry.rs (same `action_pipeline` module) can call them.
  - `retry_event_continuation` linearized: `TriggerMissing` and `FetchFailed(e.to_string())` returned as `Err(...)` instead of silently calling `save_retry_error` and returning `Ok(())`. `phase_trigger_continuation` failures `?`-propagate to the outer orchestrator match.
  - `retrigger_event_impl` mirrors `retry_last_response_impl`'s bottom match block (Cancelled → Idle reset + save_state; Err(e) → PipelineRun + `finalize_phase_error`).
  - `save_retry_error` deleted; same body lives as private `retry_persist_error` (scoped to retry.rs).
  - **Part B — 3 save-state paths reconciled** (antipattern #5): extracted private `PersistenceGate::write_snapshot(state)` helper that does `GameStateSnapshot::from_game_state` + `storage.save_snapshot`. Both `save_state` and `save_message_and_snapshot` route the snapshot row through it (was duplicated). Module doc documents the 3-caller save-path distinction (`save_message_and_snapshot` — snapshot + message insert + swipe wiring, called by message editing / arrival / `phase_finalize`; `save_state` — snapshot-only write, called by `pipeline.persist`; `save_message_and_snapshot` at arrival — injects scenario logs into `state.narrative.history` before this write).
  - 3 deliberate behavior changes: precondition "no input to retry" now sets `GenerationStatus::Error` (was silent `return;` with no state mutation); 3 B2 fetch-block tests rewritten to drive `retry_last_response_impl` (orchestrator-level seam) instead of calling `retry_event_continuation` directly — they now verify the orchestrator-level contract; `save_retry_error` body renamed to private `retry_persist_error`.
  - `python build.py` green: 1261 tests passed, 2 LLM skipped, 0 failed (down from 1263 — 2 deleted `test_save_retry_error*` tests). Cargo fmt clean, clippy clean. Coverage 86.6%.
  - LOC target ≤55 missed (body is 85 LOC) — same call-site verbosity pattern as ticket 03 (5 precondition match-arms + PipelineRun construction inside `Err(e)` arm). Linear structure achieved.
  - Map status: ticket 04 complete — T4 PhaseError consolidation destination reached (both orchestrators linearized, save-state paths reconciled, build green). Map's **Not yet specified** is empty.
...
- **Storage `require_*` helpers + canonical typed not-found errors** (archived plan: `old-docs/archived-plans/storage-require-helpers.md`). Replaced repeated required-entity `get_*.ok_or_else(...)` mappings at every required-read call site with canonical `Storage::require_game`, `require_world`, `require_persona`, `require_active_swipe_index` helpers. `get_*` methods remain unchanged for catalogue / fallback / existence / validation callers. Single commit (uncommitted for review). `python build.py` green: 1255 passed / 2 LLM skipped (log `logs/build_20260713_135819.log`).
  - **New error variants**: `EngineError::GameNotFound(u64)` (display `"Game not found: {id}"`), `EngineError::PersonaNotFound(String)` (display `"Persona not found: {key}"`), and `EngineError::MessageNotFound(u64)` (display `"Message not found: {id}"`). Existing `WorldNotFound(String)` retained as canonical for worlds; `GameNotFound(u64)` and `MessageNotFound(u64)` intentionally match the numeric IDs of their underlying `get_*` methods.
  - **Deleted variant**: `EngineError::NpcNotFound(String)` — had zero production constructors after `require_persona` migrated the last persona site in `pipeline.rs::run_from_input` and `retry.rs::retry_event_continuation`. `NpcNotFound` was semantically wrong for persona lookups (Persona ≠ Character per domain glossary) but had been overloaded for both meanings historically. Zero references remain in `chronicler_engine/src`.
  - **New helpers** (`backend/games.rs`, `backend/worlds.rs`, `backend/personas.rs`, `backend/messages.rs`): each delegates to its `get_*` sibling via `?.ok_or_else(|| <variant>())`. Backend `Err` propagates unchanged; only `Ok(None)` becomes the canonical typed not-found.
  - **Migrated required-read callers**: action-pipeline bundle load, retry event-continuation bundle load, `generation_gate::start_action`, `persistence_gate::fetch_world_data_for_fresh_state`, `persistence_gate::update_message_text` (uses new `require_active_swipe_index`), `query_handlers::get_current_room_view` / `get_npc_headshots` (now thread typed `EngineError` through `ApplicationError::From<EngineError>`), `arrival_service::run` (signature now `Result<(), EngineError>` with `#[tracing::instrument(err)]`; the three repeated `match`-and-`return` arms became `?`, matching the `?`-chains in `pipeline::run_from_input` and `retry::retry_event_continuation`; top-of-stack log emission is now span-driven), `bootstrap/run.rs` inner fallback-world lookup + persona lookup (outer requested-world lookup stays on `get_world` because `None` triggers fallback).
  - **`get_active_swipe_index` signature change**: this method previously returned `Result<usize, EngineError>` and surfaced absence as `EngineError::Config("Message {id} not found")` (InMemory) or collapsed into a generic DB error string (Sqlite). It now returns `Result<Option<usize>, EngineError>`, distinguishing absent message (`Ok(None)`) from backend error. The single required-read caller (`persistence_gate::update_message_text`) was migrated to the new `require_active_swipe_index` helper which converts `Ok(None)` to `EngineError::MessageNotFound(id)`. Existing tests updated to assert the new `Option`-shaped return.
  - **`get_*` retained**: `GameCatalogue` validation paths, `get_current_game_name` `"Unknown"` fallback, bootstrap primary-world fallback, `WorldCatalogue`, seed / existence checks.
  - **Docs**: `docs/architecture/system.md` and `docs/system/storage.md` updated with the `get_*` vs `require_*` read contract. `docs/diagnostics/error_catalog.md` adds `GameNotFound` + `PersonaNotFound` entries and removes the `NpcNotFound` entry (only the historical CHANGELOG row above retains the name as a record of the pre-migration state).
  - **Tests**: unit tests in `backend/{games,worlds,personas,swipes}_tests.rs` lock every helper (hit-returns-entity + miss-returns-canonical-variant-with-exact-payload). `src/error_tests.rs` locks canonical display strings (incl. `Message not found: {id}`). `query_handlers_tests.rs` adds two typed-error tests for missing current game. `tests/integration/bootstrap/run_branches.rs` updated to expect `PersonaNotFound` with canonical display. Pipeline + retry regression tests lock canonical missing-row status messages. Existing injected-backend-error tests on `get_world` / `get_persona` continue to prove non-absence errors flow unchanged through `?`.

## 2026-07-11

### Changed

- **T9-00 precursor: TestDataBuilder + thin TestAppBuilder + SqliteTestAppBuilder** (`.scratch/t9-world-snapshot-removal/issues/03-test-data-builder-and-testappbuilder-thin.md`). First sub-ticket of T9 (WorldSnapshot removal) effort. Thins the test-side seam before T9-01 (GameState::new → 1-arg) shrinks production code. Single commit, uncommitted for review. `python build.py` green: 1238 passed / 2 LLM skipped.
  - **Implementing ticket**: `.scratch/t9-world-snapshot-removal/issues/01-world-snapshot-dies-engine-unbundles.md`. Orchestrators fetch world data directly from `app.storage()`; `GameState` no longer bundles world-data fields. `python build.py` green: 1241 passed / 2 LLM skipped (log `logs/build_20260711_191719.log`).
  - **New types**: `TestDataBuilder` + `TestData` (`src/test_support/test_data_builder.rs`) bundle world / map / persona / NPCs as a single world-data unit; `TestData::seed_into(&storage)` replaces `seed_test_world_into_storage` (still retained as a survivor for one external caller). `TestData` derives `Clone` to support multi-app test patterns (app1 + app2 share data).
  - **`TestAppBuilder` thinned 13 → 9 fields**: removed `world`, `player`, `map`, `npcs`, `room_npcs` (replaced by `test_data: Option<TestData>`); compressed `generation_status` + `generation_phase` into single `generation: Option<(GenerationStatus, GenerationPhase)>` tuple (enforces both-together invariant). New API: `with_data(data)`, `data()`, `skip_seeding(bool)`, `build_service()`, `from_base()`. Mutator methods (`map`, `npc`, `npcs`, `room_npc`, `with_world_map`, `new(world, player)`) deleted.
  - **`SqliteTestAppBuilder`** (new, `tests/helpers/sqlite_test_app_builder.rs`): integration-only builder that replaces 5 deleted sqlite-backed helpers. Exposes `with_data`, `mock_backend`, `backends`, `separate_backends`, `game_service_fn`, `state_mut`, `message`, `messages`, `build_service`. Owns the `build_seeded_sqlite_storage` body (sqlite + `seed_default_game_row` + snapshot-save → `pre_main_id` → Input message `snapshot_id` wiring + swipe / message persistence → final snapshot re-save) and a local `finalize_app` copy. `file-level #![allow(dead_code)]` covers per-binary partial coverage (helpers file is `#[path]`-included in both `tests/integration/` and `tests/infrastructure/`).
  - **`TestAppBuilder::build_app_state` preset_storage fix**: was `Storage::new_in_memory()` (empty, no system_default preset); now `crate::test_support::default_test_preset_storage()`. Latent bug surfaced when narration tests in lib suite migrated to `TestAppBuilder` and exercised `phase_narrate → load_preset_and_response_length`.
  - **Deleted from `src/test_support/context.rs`**: `make_test_app_with_sqlite`, `make_test_app_with_mock_backend`, `make_test_app_with_backends`, `make_test_app_with_separate_backends`, `make_test_app_with_game_service`, `build_seeded_sqlite_storage` (private, orphaned), `finalize_app` (private, duplicated in `SqliteTestAppBuilder`). Deleted from `tests/helpers/fixtures.rs`: `make_test_app_with_default_preset` (dead code, 0 callers) and `app_with_storage_from` (migrated to `SqliteTestAppBuilder::game_service_fn`). `make_test_app_with_storage` was deleted in earlier T9 chunks.
  - **Deleted from `src/test_support/mod.rs` re-exports**: the 5 sqlite helpers above; kept `default_test_preset_storage`, `seed_test_world_into_storage`, `make_test_app`, `make_test_app_without_snapshot`.
  - **User-approved survivors**: `make_test_app` (1 caller in `src/application/query_handlers_tests.rs:21`), `make_test_app_without_snapshot` (2 callers: `tests/integration/application/action_pipeline/retry.rs:127`, `src/application/action_pipeline/retry_tests.rs:100` — snapshot-skip semantics not expressible in `TestAppBuilder` without a `skip_snapshot` method), `seed_test_world_into_storage` (1 external caller `tests/integration/flow/arrival_persistence.rs:139` — failing-storage test needs custom `Storage::new_in_memory().with_test_failures()` + manual `DefaultApplicationService` construction, not a sqlite builder).
  - **Guardrail updated**: `tests/infrastructure/guardrails/structure.rs:167` error string now references `SqliteTestAppBuilder` (was `make_test_app_with_sqlite()`). Underlying check is for `make_test_context(` (function never existed in codebase; remains dead guardrail — only the stale error string was updated).
  - **Validation**: `rg` returns 0 hits for all 8 deleted helpers (`make_test_app_with_storage`, `_with_sqlite`, `_mock_backend`, `_with_backends`, `_separate_backends`, `_with_game_service`, `app_with_storage_from`, `make_test_app_with_default_preset`). `TestAppBuilder` field count = 9 (≤ 9 criterion met). No production code imports `TestData` / `TestDataBuilder` (`src/application/*_tests.rs` are `#[cfg(test)]`-gated, not production).
  - **Migration scope**: ~180 call sites across 22 files migrated across chunks 2, 3a, 3b-i/ii/iii-a/b/c. Heaviest files: `tests/integration/application/game_service.rs` (22 sites), `tests/integration/flow/retry_main.rs` (9), `src/application/action_pipeline/pipeline_tests.rs` (13), `src/application/action_pipeline/retry_tests.rs` (14), `tests/integration/flow/sequence.rs` (10).
  - **Docs**: `docs/reference/test_support.md` rewritten to cover `TestDataBuilder` + `TestAppBuilder` + `SqliteTestAppBuilder` and remove all deleted-helper references. `docs/reference/testing.md` Backend Selection section updated.
  - **Post-implementation review fixes** (`tmp/review-t9-00-test-support-2026-07-11.md`): 3 apply-now items from dual-lens review. (1) **`seed_test_world_into_storage` now delegates to `TestData::seed_into`** — survivor body reduced from 17 to 10 lines; builds `TestData` from `&GameState` fields, calls `data.seed_into(storage)`; fail-loud `.expect()` lives in `test_data_builder.rs` (single source of truth). Resolves divergent error semantics (originally `.unwrap_or(1)` silent-mask). arch-lint `no-unwrap-expect [AL001]` blocks `.expect()` in `context.rs` (filename lacks "test"); `test_data_builder.rs` (filename has "test") is exempt via `allow_in_tests = true`. (2) **Dropped `+ Send` bound on `state_mut` closure** in `SqliteTestAppBuilder` — verified unused (synchronous invocation inside `build_service`; no spawn sites). (3) **Deleted `BackendSpec::Default` variant** — 10 callers in `tests/integration/application/game_service.rs` migrated to add `.backends(MockBackend::default)` (matches neighbor file pattern); `backend` field now `Option<BackendSpec>` (>/None< arm panics with clear message). Full review analysis + deferred items (A2 typed setters + A4 `to_transient_state` extraction as follow-up ticket 04; A5 `Message::input/system/narration` factories as separate domain ticket; A6 `tests/_support/` crate move + A7 `find_free_fn_smells.py` governance rejected) at ticket `## Answer` Post-implementation review section. `python build.py` green: 1238 passed / 2 LLM skipped (log `logs/build_20260711_171713.log`).

- **T13: T9-01 Tier-1 Cleanup + Dynamic-Rooms Bug Fix** (archived plan: `old-docs/archived-plans/t13-t9-01-tier-1-cleanup-dynamic-rooms-bug-fix.md`). 9 items from T9-01 review triage: 1 latent correctness bug + 8 mechanical type/hygiene/dead-code cleanups. Single commit (uncommitted for review). `python build.py` green: 1242 passed / 2 LLM skipped (log `logs/build_20260711_232549.log`).
  - **Bug fix (Phase 1)**: `src/application/action_pipeline/phases.rs` `phase_narrate` (line 99) — added `.or_else(dynamic_rooms)` fallback so dynamic rooms (created by `create_dynamic_room` during semantic walk) resolve correctly. Was real correctness bug: 4 of 5 inline `current_room()` lookup sites had the fallback; this one didn't. Tests didn't catch it because no test exercised `phase_narrate` when `current_room_id` was dynamic. New regression test `phase_narrate_resolves_dynamic_room_via_fallback` in `pipeline_tests.rs` locks the fix.
  - **Type alignment (Phase 2)**: `fetch_world_bundle` (`pipeline.rs`) and `fetch_world_bundle_for_retry` (`retry.rs`) return `Result<_, EngineError>` instead of `String` / `Option`. Missing-world → `EngineError::WorldNotFound`, missing-persona → `EngineError::NpcNotFound`, missing-game → `EngineError::Internal`. Dropped `type WorldBundle` alias; added `#[allow(clippy::type_complexity)]` on `fetch_world_bundle` signature. `fetch_world_bundle_for_retry` is now a pure fetch — `save_retry_error` side-effect moved to caller `retry_event_continuation`. Regression tests assert on **property** (`GenerationStatus::Error(_)` was set) instead of message-word string-match.
  - **Post-plan follow-up (A2)**: inlined both `fetch_world_bundle` and `fetch_world_bundle_for_retry` at their single caller sites (`run_from_input` + `retry_event_continuation`). Removing the bundling (4-tuple `Result` return) restores the T9-01 intent — `WorldSnapshot` / `WorldBundle` was meant to die, these helpers were circumventing that by re-bundling world+map+persona+npcs into a single `Result` object. Both helpers just did 4 DB calls anyway; callers now do 4 direct `app.storage().get_game/get_world/get_persona/list_characters` calls inline via a try-closure pattern. Consolidated `retry_event_continuation`'s duplicate `get_game` call (was fetching game twice for `(map, npcs)` and `persona` blocks — now single fetch). Side-effect move preserved: caller still calls `save_retry_error` on Err before early-returning `ActionOutcome::Completed`. Test `test_fetch_world_bundle_for_retry_returns_err_on_world_fetch_error` was testing a now-deleted helper; re-purposed as `test_retry_event_continuation_returns_completed_on_world_fetch_failure` exercising the full retry path with `get_world` override and asserting property-match (status == `GenerationStatus::Error`). Existing persona-failure test also updated to property-match per Issue 3 decision.
  - **Mechanical cleanup (Phase 3)**: Deleted 5 dead helpers in `tests/helpers/fixtures.rs` (`create_test_game_state`, `create_navigation_test_map`, `create_simple_test_map`, `create_basic_test_state`, `create_basic_test_state_no_scenario`). Merged 2 misleading helper names in `tests/helpers/pipeline_helpers.rs` (`create_test_state_with_map` + `create_test_state_with_trigger_npc`, identical bodies) into single `create_minimal_test_state()` — 25 callers updated across 5 files. Replaced `unwrap_or(0)` → `expect("snapshot save must succeed in test builder")` at `tests/helpers/sqlite_test_app_builder.rs:223`. Deleted `find_room_in_map` + `find_room_in_world_map` one-line wrappers in `src/domain/engine/logic.rs`; inlined `map.get_room_by_id(...)` at 2 production callers (`scenario.rs:24`, `logic.rs` `attempt_semantic_walk`); deleted 3 trivial wrapper tests (`test_get_room_by_id_missing`, `test_attempt_walk_dangling_exit`, `test_get_room_by_id_existing`). Dropped unused args from 3 `action_processing.rs` functions: `_map` from `update_npc_encounters_on_room_change`, `_npcs` from `log_movement_completion`, `_world` from `execute_freeaction_impl`. 7 callers updated in `phases.rs`, `action_processing_tests.rs`, `invariant_contract.rs`. `EngineDeps` struct in `action_processing_tests.rs` lost its `world: Arc<WorldCard>` field.
  - **Out of scope** (deferred per user): Tier-2 #9 (retry double-fetch), #11 (QuantifierCtx struct), #12 (seed_test_world_into_storage test fidelity).
  - **Docs**: `docs/reference/test_support.md:115` row updated to drop deleted `create_basic_test_state` / `create_basic_test_state_no_scenario`.
